 ...
 * @version $Id: engine_epoll.c 1933 2010-01-04 03:24:36Z entrope $
 */
#include "config.h"
 ...
 *
 * NOTE: The _syscall1/_syscall4 fallback was removed in Cathexis 1.1.0.
 * Every Linux system since glibc 2.4 (2006) provides epoll_create,
 * epoll_ctl, and epoll_wait as proper library functions via <sys/epoll.h>.
 * If your system somehow lacks them, upgrade glibc.
 */
#if defined(EPOLL_NEED_BODY)
/* Configure detected that epoll functions couldn't be linked during the
 * test, but this is almost always a false negative caused by profiling
 * flags (-pg) or pedantic mode breaking the link test.  The functions
 * are in <sys/epoll.h> which we already include above.  If they truly
 * don't exist, the build will fail here with a clear error. */
#endif /* EPOLL_NEED_BODY */
